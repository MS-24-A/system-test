...
import { UPDATE_PRODUCT } from '../../../apollo/graphql/mutations';
...
const ProductsEditForm = ({ loading, setLoading, changeReloading, handlePopUpForm, aditionalData }) => {

    const [updateProduct, __] = useMutation(gql(UPDATE_PRODUCT));
    ...
    useEffect(() => {
        ...
        if (name != null && name.length > 0 && (name != actualName)) {
            setCorrectName(true);
            setErrorName(false);
        } else {
    ...
    const assingActualValues = async () => {
        setActualName(aditionalData?.dataToEdit?.name?.trim() || '');
        setActualPrice(aditionalData?.dataToEdit?.price || '');
        setName(aditionalData?.dataToEdit?.name?.trim() || '');
        setPrice(aditionalData?.dataToEdit?.price || '');
        setStatus(aditionalData?.dataToEdit?.status || '');
    }
    ...
    const handleEdit = async (e) => {
        ...
        const { data } = await updateProduct({
            variables: {
                product: {
                    name: name,
                    price: Number(price),
                    id: aditionalData?.dataToEdit?.id
                }
            }
        });
        ...
        if (data.updateProduct.error) {
            setTimeout(() => {
                ...
                setMessage(data.updateProduct.message || "Error en el servidor.");
            }, 800);
        } else {
            setMessage(data.updateProduct.message);
